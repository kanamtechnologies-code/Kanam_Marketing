...
export function SubpageShell({
  ...
  return (
    <div className="relative isolate -mt-8 overflow-hidden bg-background md:-mt-20">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[92vw] px-5 py-10 sm:px-6 lg:px-10 md:py-14">
        {title ? (
        ...
        <div className={toc?.length ? "mt-10 grid gap-10 md:grid-cols-[220px_1fr]" : "mt-10"}>
          {toc?.length ? (
          ...
          <main className="min-w-0 [&_p]:text-base [&_p]:md:text-lg [&_p]:leading-relaxed [&_li]:text-base [&_li]:md:text-lg [&_li]:leading-relaxed [&_h3]:text-lg [&_h3]:md:text-xl [&_h3]:font-semibold">
            {children}
          </main>
        </div>
      </div>
...
export function Band({ className, children }: { className?: string; children: ReactNode }) {
  return (
    <div
      className={cn(
        "rounded-3xl border border-foreground/15 bg-white/72 p-6 shadow-[0_12px_34px_rgba(15,23,42,0.10)] ring-1 ring-[rgb(var(--accent-rgb)/0.14)] backdrop-blur-sm transition-shadow hover:shadow-[0_16px_40px_rgba(15,23,42,0.13)] dark:bg-background/55",
        className
      )}
    >
      {children}
